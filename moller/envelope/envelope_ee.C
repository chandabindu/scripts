...
void set_plot_style();
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &det28trid, std::vector<int> &col2trid, std::vector<int> &col4trid);
...
int envelope_ee(TString source, TString out)
{
TChain T("T");
T.Add(Form("%s",source.Data()));
/*const Int_t nfile=210;
TString add_file[nfile]={""};
for(int ij=1;ij<=nfile;ij++){
ostringstream string1;
string1<<Form("/lustre19/expphy/volatile/halla/moller12gev/chandan/sim_out/envelope_ee/remollout_ee%d.root",ij);
...
//for(size_t j=0;j<1000;j++){
  ...
  isValid1(fHit,det28trid,col2trid,col4trid);
  ...
  for(size_t i=0;i<fHit->size();i++){
    ...
    if(!(det28it!=det28trid.end() && col2it!=col2trid.end() && col4it!=col4trid.end())) {/*std::cout<<"Inside Continue loop::Event "<<j<<" hit "<<i<<" det "<<det<<" det28trid size "<<det28trid.size()<<" col2 size "<<col2trid.size()<<" col4exit size "<<col4trid.size()<<std::endl;*/continue;}
    //std::cout<<"After Continue :: Event "<<j<<" hit "<<i<<" trid "<<trid<<" iterator find "<<std::endl;  
    if(((det>=57 && det<=79) || (det>=85 && det<=91) || det==28 ||det==29 ) && pid==11 && hit.e>500){
...
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &det28trid, std::vector<int> &col2trid, std::vector<int> &col4trid)
{
  for(size_t i=0;i<fHit->size();i++){
    ...
    if(det==28 && hit.r>=640 &&hit.r<=1195 && abs(pid)==11 && hite>500){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	det28trid.push_back(hit.trid);
    }
    if(det==59 && hit.r>=35 && hit.r<=101 && abs(pid)==11 && hite>500){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	col2trid.push_back(hit.trid);
    }
    if(det==65 && hit.r>=50 && hit.r<=197 && abs(pid)==11 && hite>500){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
