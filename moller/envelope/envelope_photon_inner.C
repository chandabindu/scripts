/*
This program is written to calculate the inner photon envelope 
...
int envelope_photon_inner(TString source, TString out)
{
TChain T("T");
...
for(Int_t k=0;k<nDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  ...
  if(k<=8){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),40,-30.,10.,20,-5,15);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),30,0,30);
 }
  else if(k>=9 && k<=24){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),80,-70.,10.,35,-5,30);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),70,0,70);
 }
  else if(k>=25 && k<=31){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),150,-140.,10.,65,-5,60);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),140,0,140);
 }
 }
}
