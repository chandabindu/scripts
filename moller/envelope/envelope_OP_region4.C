/*
This program is written to calculate envelope for sections between Lintel and Collar1 
Chandan Ghosh Dec 2020

*/
#include "remolltypes.hh"
void set_plot_style();
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &col2trid, std::vector<int> &Linteltrid, std::vector<int> &Collar1trid);
...
int envelope_OP_region4(TString source, TString out)
{
TChain T("T");
...
std::map<int,string> snDet  {{76,"Lintel"},{77,"DSCoil12"},{78,"DSCoil13"},{79,"DSCoil14"},{85,"DSCoil15"},{86,"DSCoil16"},{87,"Collar1"}};
string sDet[nDet] = {"Lintel","DSCoil12","DSCoil13","DSCoil14","DSCoil15","DSCoil16","Collar1"};


for(Int_t k=0;k<nDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  part=Form("h_%s_%s",sDet[k].c_str(),sParticle[i].c_str());
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),650,-700.,-50.,200,-5,195);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),130,0,650);
 }
}
...
//for(size_t j=0;j<1000;j++){
  ...
  std::vector<int> Collar1trid;
  std::vector<int>::iterator Collar1it;
  if(fRate==0) fRate=1;
  //isValid(fHit,&test1,&test2,&test3,&test4);
  isValid1(fHit,col2trid,Linteltrid,Collar1trid);
  ...
  for(size_t i=0;i<fHit->size();i++){
    ...
    Collar1it = find(Collar1trid.begin(),Collar1trid.end(),trid);
    if(!(Collar1it!=Collar1trid.end() && col2it!=col2trid.end() && Lintelit!=Linteltrid.end())) {/*std::cout<<"Inside Continue loop::Event "<<j<<" hit "<<i<<" det "<<det<<" det28trid size "<<det28trid.size()<<" col2 size "<<col2trid.size()<<" col4exit size "<<col4trid.size()<<std::endl;*/continue;}
    //std::cout<<"After Continue :: Event "<<j<<" hit "<<i<<" trid "<<trid<<" iterator find "<<std::endl;  
    if(((det>=76 && det<=79) || (det>=85 && det<=87))){
...
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &col2trid, std::vector<int> &Linteltrid, std::vector<int> &Collar1trid)
{
  for(size_t i=0;i<fHit->size();i++){
    ...
    if(det==76 && hit.r>=116 && hit.r<=435 && abs(pid)==11 && hite>500 && hit.vz<=-3875){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	Linteltrid.push_back(hit.trid);
    }
    if(det==87 && abs(pid)==11 && hite>500 && hit.vz<=-3875){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	Collar1trid.push_back(hit.trid);
    }
 }
