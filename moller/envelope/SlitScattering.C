...
void set_plot_style();
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &det28trid, std::vector<int> &col2trid, std::vector<int> &col4trid, std::vector<int> &CutPlanetrid);
...
int SlitScattering(TString source, TString out)
{
TChain T("T");
T.Add(Form("%s",source.Data()));
/*const Int_t nfile=50;
...
std::map<int,string> snDet  {{57,"tgtPbWall"},{58,"tgtPbCollar"},{59,"Col2Ent"},{60,"USCoil1"},{61,"USCoil2"},{62,"USCoil3"},{63,"USCoil4"},{64,"USCoil5"},{65,"Col4Ent"},{66,"PbWall"},{67,"DSCoil2"},{68,"DSCoil3"},{69,"DSCoil4"},{70,"DSCoil5"},{71,"DSCoil6"},{72,"DSCoil7"},{73,"DSCoil8"},{74,"DSCoil9"},{75,"DSCoil10"},{76,"Lintel"},{77,"DSCoil12"},{78,"DSCoil13"},{79,"DSCoil14"},{85,"DSCoil15"},{86,"DSCoil16"},{87,"Collar1"},{88,"Drift"},{89,"Collar2"},{91,"GEM4"},{28,"maindet"},{29,"piondet"},{90,"HallExit"}};
...
for(Int_t k=0;k<nDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  ...
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),500,-500.,0.,210,-10,200);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),48,60,500);
 }
  else if(k>=21 && k<=25){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),570,-670.,-100.,310,-10,300);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),114,100,670);
 }
  else if(k>=26 && k<=27){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),650,-1000.,-350.,630,-30,600);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),120,400,1000);
 }
  else if(k>=28 && k<=29){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),770,-1250.,-480.,730,-30,700);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),150,550,1300);
 }
  else if(k>=30 && k<=31){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),1000,-1600.,-600.,930,-30,900);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),180,700,1600);
  }
 }
}
...
//for(size_t j=0;j<1000;j++){
  ...
  std::vector<int> CutPlanetrid;
  std::vector<int>::iterator CutPlaneit;
  if(fRate==0) fRate=1;
  isValid1(fHit,det28trid,col2trid,col4trid,CutPlanetrid);
  ...
  for(size_t i=0;i<fHit->size();i++){
    ...
    CutPlaneit = find(CutPlanetrid.begin(),CutPlanetrid.end(),trid);

    if(!(det28it!=det28trid.end() && col2it!=col2trid.end() && col4it!=col4trid.end() && CutPlaneit!=CutPlanetrid.end())) {/*std::cout<<"Inside Continue loop::Event "<<j<<" hit "<<i<<" det "<<det<<" det28trid size "<<det28trid.size()<<" col2 size "<<col2trid.size()<<" col4exit size "<<col4trid.size()<<std::endl;*/continue;}
    //std::cout<<"After Continue :: Event "<<j<<" hit "<<i<<" trid "<<trid<<" iterator find "<<std::endl;  
    if(((det>=57 && det<=79) || (det>=85 && det<=91) || det==28 || det==29) && pid==11){
//	std::cout<<"Hi Event "<<j<<" hit "<<i<<" det "<<det<<" pid "<<pid<<" r "<<hit.r<<" trid "<<hit.trid<<" mtrid  "<<hit.mtrid<<" phi "<< (180./acos(-1))*atan(hit.y/hit.x)<<std::endl;
		...
		h_d_rz[part]->Fill(hit.vz,sqrt(hit.vx*hit.vx+hit.vy*hit.vy),(fRate)*weight*hite);
		h_d_r[part]->Fill(hit.r,(fRate)*weight*hite);
		h_d_p[part]->Fill(hit.p,(fRate)*weight*hite);
    }
  }
}
...
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &det28trid, std::vector<int> &col2trid, std::vector<int> &col4trid, std::vector<int> &CutPlanetrid)
{
  for(size_t i=0;i<fHit->size();i++){
    ...
    if(det==28 && hit.r>=640 &&hit.r<=1195 && abs(pid)==11){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	det28trid.push_back(hit.trid);
    }
    if(det==59 && hit.r>=35 && hit.r<=101 && abs(pid)==11){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	col2trid.push_back(hit.trid);
    }
    if(det==65 && hit.r>=50 && hit.r<=197 && abs(pid)==11){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
