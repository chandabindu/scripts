/*
This program is written to plot scattered plots for outer photon envelopes. 
...
int envelope_photon_outer(TString source, TString out)
{
TChain T("T");
...
string1<<Form("/lustre19/expphy/volatile/halla/moller12gev/chandan/sim_out/envelope_photon_newgeo/remollout_phonton%d.root",ij);
...
for(Int_t k=0;k<nDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  ...
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),620,-670.,-50.,310,-10,300);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),124,50,670);
 }
  else if(k>=26 && k<=27){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),830,-900.,-70.,630,-30,600);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),166,70,900);
 }
  else if(k>=28 && k<=29){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),900,-1000.,-100.,730,-30,700);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),180,100,1000);
 }
  else if(k>=30 && k<=31){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),1100,-1200.,-100.,930,-30,900);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),220,100,1200);
  }
 }
}
...
//for(size_t j=0;j<1000;j++){
  ...
  for(size_t i=0;i<fHit->size();i++){
    ...
    if(((det>=57 && det<=79) || (det>=85 && det<=91) || det==28 ||det==29 ) && pid==11 && hit.e>10){
...
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &col2trid, std::vector<int> &col4trid)
{
  for(size_t i=0;i<fHit->size();i++){
    ...
    if(det==59 && hit.r>=35 && hit.r<=101 && abs(pid)==11 && hite>10){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	col2trid.push_back(hit.trid);
    }
    if(det==65 && hit.r>=50 && hit.r<=197 && abs(pid)==11 && hite>10){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
