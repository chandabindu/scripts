/*
This program is written to calculate envelopes from Hybrid wall to lintels 
Chandan Ghosh March 2021

*/
#include "remolltypes.hh"
void set_plot_style();
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &col2trid, std::vector<int> &PbWalltrid, std::vector<int> &Linteltrid);
...
int envelope_OP_region3(TString source, TString out)
{
TChain T("T");
...
std::map<int,string> snDet  {{66,"PbWall"},{67,"DSCoil2"},{68,"DSCoil3"},{69,"DSCoil4"},{70,"DSCoil5"},{71,"DSCoil6"},{72,"DSCoil7"},{73,"DSCoil8"},{74,"DSCoil9"},{75,"DSCoil10"},{76,"Lintel"}};
string sDet[nDet] = {"PbWall","DSCoil2","DSCoil3","DSCoil4","DSCoil5","DSCoil6","DSCoil7","DSCoil8","DSCoil9","DSCoil10","Lintel"};
...
for(Int_t k=0;k<nDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  ...
  if(k<=0){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),350,-340.,10.,165,-5,160);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),70,0,350);
 }
  else if(k>=1 && k<=10){
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("xy dist. at %s for %s",sDet[k].c_str(),sParticle[i].c_str()),500,-500.,0.,210,-10,200);
  h_d_r[part] = new TH1D(part+"_d_r",Form("R dist. at  %s for %s",sDet[k].c_str(),sParticle[i].c_str()),100,0,500);
 }
 }
}
...
//for(size_t j=0;j<1000;j++){
  ...
  std::vector<int> Linteltrid;
  std::vector<int>::iterator Lintelit;
  if(fRate==0) fRate=1;
  //isValid(fHit,&test1,&test2,&test3,&test4);
  isValid1(fHit,col2trid,PbWalltrid,Linteltrid);
  ...
  for(size_t i=0;i<fHit->size();i++){
    ...
    Lintelit = find(Linteltrid.begin(),Linteltrid.end(),trid);
    if(!(col2it!=col2trid.end() && PbWallit!=PbWalltrid.end() && Lintelit!=Linteltrid.end())) {/*std::cout<<"Inside Continue loop::Event "<<j<<" hit "<<i<<" det "<<det<<" det28trid size "<<det28trid.size()<<" col2 size "<<col2trid.size()<<" col4exit size "<<col4trid.size()<<std::endl;*/continue;}
...
void isValid1(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &col2trid, std::vector<int> &PbWalltrid, std::vector<int> &Linteltrid)
{
  for(size_t i=0;i<fHit->size();i++){
    ...
    if(det==66 && hit.r<=270 && abs(pid)==11 && hite>500 && hit.vz<=-3875){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	PbWalltrid.push_back(hit.trid);
    }
    if(det==76 && abs(pid)==11 && hite>500 && hit.vz<=-3875){
	//std:;cout<<"Inside isvalid1 det "<<det<<" r "<<hit.r<<"   track id "<<hit.trid<<std::endl;
	Linteltrid.push_back(hit.trid);
    }
 }
