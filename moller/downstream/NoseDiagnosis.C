...
bool isValid(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &hittrid);
...
string1<<Form("/lustre19/expphy/volatile/halla/parity/chandan/sim_out/DSConfig0Shield10_beam/remollout_beam%d.root",ij);
...
TFile f("noseDSConfig0Shield10_beam_new.root","RECREATE");
set_plot_style();
...
std::map<int,string> snDet  {{63,"afterCol4"},{73,"first"},{76,"second"},{64,"beforeCoil1"}};//,{65,"afterCoil1"},{66,"afterCoil2"},{72,"beforeLintel"},{67,"afterCoil3"},{73,"At13M"},{77,"At13p1M"},{76,"At13p4M"},{78,"At13p5M"},{74,"At14M"},{79,"At14p1M"},{75,"At14p5M"},{71,"At14p6M"},{68,"beforeCollar"},{69,"driftRegion"},{28,"maindet"}};
string sDet[nDet] = {"afterCol4","first","second","beforeCoil1"};//,"afterCoil1","afterCoil2","beforeLintel","afterCoil3","At13M","At13p1M","At13p4M","At13p5M","At14M","At14p1M","At14p5M","At14p6M","beforeCollar","driftRegion","maindet"};
...
for(Int_t k=0;k<nDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  ...
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("%s on %s",sParticle[i].c_str(),sDet[k].c_str()),500,-250, 250, 500,-250,250);
  }
  for(int j=0;j<nEnergy && i>0;j++){
  part=Form("h_%s_%s_E%d",sDet[k].c_str(),sParticle[i].c_str(),j);
  h_d_xy[part] = new TH2D(part+"_d_xy",Form("%s %s for %s",sEnergy[j].c_str(),sParticle[i].c_str(),sDet[k].c_str()),500,-250, 250, 500,-250,250);
  h_d_r[part] = new TH1D(part+"_d_r",Form("Radial::%s %s for %s",sEnergy[0].c_str(),sParticle[i].c_str(),sDet[k].c_str()), 250,0,250);
  }
  part1=Form("h_%s_%s",sDet[k].c_str(),sParticle[i].c_str());
  //std::cout<<"Test inside defination loop:1D  "<<part1<<std::endl;
  h_d_e[part1] = new TH1D(part1+"_d_e",Form("%s %s for %s",sEnergy[0].c_str(),sParticle[i].c_str(),sDet[k].c_str()), 120,0,12000);
 }
}
...
for(Int_t k=0;k<nTDet;k++){
 for(Int_t i=0;i<nParticle;i++){
  ...
  //std::cout<<"Test inside defination loop:i==0  "<<part<<std::endl;
  h_t_rz[part] = new TH2D(part+"_t_rz",Form("%s on %s",sParticle[i].c_str(),sTDet[k].c_str()),550,3300, 5500, 90,0,100);
 /* if(k>=7){
   part=Form("h_%s",sDet[k].c_str());
...
//  std::cout<<"Inside the histogram defination "<<part<<std::endl;
  h_t_rz[part] = new TH2D(part+"_t_rz",Form("%s %s for %s",sEnergy[j].c_str(),sParticle[i].c_str(),sTDet[k].c_str()),550,3300, 5500, 90,0,100);
  ...
  if(k==0){//-180 to +180deg phi angle covers all the coils. So define the histograms once is enough!!
 	part2=Form("h_%s_E%d",sParticle[i].c_str(),j);
        h_t_phiz[part2] = new TH2D(part2+"_t_phiz",Form("On Cylindrical %s for %s",sEnergy[j].c_str(),sParticle[i].c_str()),550,3300, 5500, 380,-190,190);
  }
  }
  part1=Form("h_%s_%s",sTDet[k].c_str(),sParticle[i].c_str());
  //std::cout<<"Test inside defination loop:1D  "<<part1<<std::endl;
  h_t_e[part1] = new TH1D(part1+"_t_e",Form("%s %s for %s",sEnergy[0].c_str(),sParticle[i].c_str(),sTDet[k].c_str()), 120,0,12000);
...
//for(size_t j=0;j<100;j++){
  ...
  /*if(fRate==0)*/ fRate=1;
  test = isValid(fHit,hittrid);
  if(!test) continue;
  //std::cout<<"After event number "<<j<<" test "<<test<<std::endl;
  for(size_t i=0;i<fHit->size();i++){
    ...
    it=find(hittrid.begin(),hittrid.end(),trid);
//std::cout<<"Event "<<j<<" hit "<< i<<" pointer trid value from vector "<<hittrid.size()<<" hit.trid "<<hit.trid<<std::endl;
    if(it == hittrid.end()){ continue;}
   // std::cout<<"Before Event "<<j<<" hit "<<i<<" det "<<det<<" pid "<<pid<<" r "<<hit.r<<" trid "<<hit.trid<<" mtrid  "<<hit.mtrid<<std::endl;
    ...
    if(((det>=57 && det<=64) || det==73 || det==76) && (abs(pid)==11 || pid==22)){// && hit.trid==hittrid[0]){
//std::cout<<"Hi Event "<<j<<" hit "<<i<<" det "<<det<<" pid "<<pid<<" r "<<hit.r<<" trid "<<hit.trid<<" mtrid  "<<hit.mtrid<<" matched it "<<*it<<std::endl;
        Int_t hitz = hit.z;
	if(det==63 || det==64 || det==73 || det==76){
...
bool isValid(std::vector<remollGenericDetectorHit_t> *fHit, std::vector<int> &hittrid)
{
  ...
  for(size_t i=0;i<fHit->size();i++){
    ...
    int det = hit.det; 
    int pid = hit.pid;
    if(!((abs(pid)==11 || pid==22) && ((det>=3008 && det<=3014) || det==63))) continue;
    //std::cout<<"Inside isvalid det "<<det<<" r "<<hit.r<<" pid "<<pid<<" z "<<hit.z<<std::endl;	
    if(det==63 && hit.r<200) {hittrid.push_back(hit.trid); test1=true;} 
    if((det>=3008 && det<=3014) && hit.r>=40 && hit.r<=50 && hit.z>=4950 && hit.z<=5140) {hittrid.push_back(hit.trid); test2=true;}// && hit.r<=1075) test2=true; 
    if(test1&&test2){
	//hittrid.push_back(hit.trid);
	//std::cout<<"Inside isvalid IfTRUE det "<<det<<" r "<<hit.r<<" pid "<<pid<<" z "<<hit.z<<" trid "<<hit.trid<<" size "<< hittrid.size()<<std::endl;	
 return true;}
   }
