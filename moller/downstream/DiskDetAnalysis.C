...
TFile f("DSConfig0Shield_ep_disk_28.root","RECREATE");
set_plot_style();
...
std::map<int,string> snDet  {{63,"afterCol4"},{64,"beforeCoil1"},{65,"afterCoil1"},{66,"afterCoil2"},{67,"afterCoil3"},{68,"beforeCollar"},{69,"driftRegion"},{28,"maindet"}};
...
//for(size_t j=0;j<100;j++){
  ...
  for(size_t i=0;i<fHit->size();i++){
   ...
   // std::cout<<"Before Event "<<j<<" hit "<<i<<" det "<<det<<" pid "<<pid<<" r "<<hit.r<<" trid "<<hit.trid<<" mtrid  "<<hit.mtrid<<std::endl;
    ...
    if(((det>=57 && det<=69) || det==28 ) && (abs(pid)==11) && (hit.trid==1 && hit.mtrid==0) && hit.pz>0){
//std::cout<<"Hi Event "<<j<<" hit "<<i<<" det "<<det<<" pid "<<pid<<" r "<<hit.r<<" trid "<<hit.trid<<" mtrid  "<<hit.mtrid<<std::endl;
        Int_t hitz = hit.z;
	if((det>=63 && det<=69) || det==28){
